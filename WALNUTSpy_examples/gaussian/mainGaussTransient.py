...
hmins = np.zeros((nrep,len(ds),3,nstep))
hmaxs = np.zeros((nrep,len(ds),3,nstep))
...
for dd in ds:
    ...
    for rep in range(0,nrep):
        ...
        hmaxs[rep,k,0,:] = H*(2**(-diagnostics[:,8]))
        hmins[rep,k,0,:] = H*(2**(-diagnostics[:,9]))
        ...
        hmaxs[rep,k,1,:] = H*(2**(-diagnostics[:,8]))
        hmins[rep,k,1,:] = H*(2**(-diagnostics[:,9]))
    ...
    np.save("gaussTransientHmin",hmins)
    np.save("gaussTransientHmax",hmaxs)
